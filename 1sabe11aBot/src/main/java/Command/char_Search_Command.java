...
//                        String rs = charName.charNameSearch(args[0],Integer.parseInt(args[1]));
                        ...
                        searchTable.search(args[0],Integer.parseInt(args[1]));
//                        testsearch = new testSearch(args[0],Integer.parseInt(args[1]));
//                        tc.sendMessage(testsearch.radio()).queue();  //무기정보
                        try{
                            if(searchTable.getBuffTable(38).equals("라우스") || searchTable.getBuffTable(38).equals("최후의")){
                                eb.setTitle("眞:크루세이더("+args[0]+")",searchTable.getThumbnail_URL())
                                    .addField("종합 (2각)", searchTable.getBuffTable(47) + " "+searchTable.getBuffTable(48), true)
                                    .addField("버프 점수", searchTable.getBuffTable(57), true)
//                                    .addField("랭킹",searchTable.getRankTable(7),false) 랭킹
                                    .setColor(Color.GREEN);
                            } else {
                                eb.setTitle("眞:헤카테("+args[0]+")",searchTable.getThumbnail_URL())
                                	.addField("종합 (편애)", searchTable.getBuffTable(38) + " " + searchTable.getBuffTable(39), true)
                                    .addField("버프 점수(편애X)", searchTable.getBuffTable(49) + " " +searchTable.getBuffTable(50), true)
//                                    .addField("랭킹",searchTable.getRankTable(7),false) 랭킹
                                    .setColor(Color.PINK);
                            }
...
//                                    .addField("랭킹",searchTable.getRankTable(3),true)
                            		.addField("시로코1시 25초x",searchTable.getDmgTable(14),true)
                                    .addField("시로코1시 25초",searchTable.get1sinergeDmgTable(),true)
                                    .setColor(Color.BLUE);
                        }
                        eb.setImage("https://img-api.neople.co.kr/df/servers/"+serverCode.selectServer(Integer.parseInt(args[1]))+
                                "/characters/"+charName.charNameSearch(args[0],Integer.parseInt(args[1]))+"?zoom=0").addBlankField(true);
